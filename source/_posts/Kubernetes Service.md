...
```mermaid
graph LR
    Client([Internal Client]) -.->|Cluster Internal| SVC[Service : ClusterIP]
    SVC -->|Load Balancing| Pod1(Pod 1)
    SVC -->|Load Balancing| Pod2(Pod 2)
    SVC -->|Load Balancing| Pod3(Pod 3)
...
```mermaid
graph TD
    Client([External Client]) -->|NodeIP : NodePort| Node1[Node 1]
    Client -->|NodeIP : NodePort| Node2[Node 2]
    
    Node1 -.->|iptables/IPVS| SVC[Service : ClusterIP]
    Node2 -.->|iptables/IPVS| SVC
    ...
    classDef k8s fill:#326ce5,stroke:#fff,stroke-width:2px,color:#fff;
    classDef pod fill:#e8f5e9,stroke:#388e3c,color:#000;
    classDef node fill:#eceff1,stroke:#475569,color:#000;
    class SVC k8s;
    class Pod1,Pod2 pod;
...
sequenceDiagram
    ...
    CoreDNS->>API: 1. Watch for Service/Endpoint changes
    API-->>CoreDNS: 2. Notify new Service created
    CoreDNS->>CoreDNS: 3. Update internal DNS records (A/AAAA/SRV)
    
    Pod->>CoreDNS: 4. DNS Query (my-svc.my-namespace.svc.cluster.local)
    CoreDNS-->>Pod: 5. Return Service ClusterIP
    Pod->>Pod: 6. Connect to ClusterIP
```
